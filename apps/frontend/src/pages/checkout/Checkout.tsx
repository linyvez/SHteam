...
const Checkout = () => {
  ...
  const platformFee = isFree ? 0 : 0.5;
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        alert("Purchase successful! Your shader will be added to the library shortly.");
        navigate("/library");
      }, 1500);
    } catch (err) {
      ...
      <div className="w-full max-w-3xl mx-auto bg-shteam-comp rounded-xl shadow-2xl border border-gray-800 p-8 mt-10 flex flex-col md:flex-row gap-8">
        <div className="flex-1 flex flex-col items-center gap-4">
          <img
            src={
              shader.thumbnailUrl ||
              "https://placehold.co/400x400/1a1a1a/444444?text=No+Preview"
            }
            alt={shader.title}
          ...
          <div>
            ...
            <div className="mt-6 flex flex-col gap-2">
              <div className="flex justify-between items-center text-sm">
                ...
                <span
                  className={`font-mono font-bold ${canAfford ? "text-green-400" : "text-red-400"}`}
                >
              ...
              {!canAfford && (
                <div className="alert alert-error mt-2 shadow-sm rounded-lg p-3">
                  <span className="text-sm font-semibold">
                    Insufficient funds. Please top up your wallet in your
                    profile.
                  </span>
                </div>
              )}
