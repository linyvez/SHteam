...
const Library = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await fetch(`http://localhost:3001/api/orders/history?userId=${userId}`);
        ...
        const fetchShaders = async () => {
            try {
                ...
                for (const order of orders) {
                    ...
                    const response = await fetch(`http://localhost:3000/api/catalog/shaders/${order.shader_id}`)
